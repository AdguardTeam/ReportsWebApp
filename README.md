...
| `filters` | string | `2.3.4.224` | Dot-separated list of filter IDs | All | Divider `.` |
...
| `userscripts` | string | Comma-separated list | Enabled userscripts or Android extensions equivalent | All | Divider `,` |
| `dns.enabled` | bool | `true` / `false` | Whether DNS protection or proxy module is enabled | All | |
| `dns.filters` | string | Desktop: `2.3.4` (IDs), Mobile: URLs | DNS filters applied | All | Platform-dependent format |
| `dns.servers` | string | Desktop: IPs/System, Mobile: URLs | Active DNS servers | All | Divider `,` |
...
| `extensions.enabled` | bool | `true` / `false` | Whether extensions are enabled | Win, Mac | |
...
| `is_third_party_cookies` | bool | `true` / `false` | Self-destructing third-party cookies flag | And | |
| `is_first_party_cookies` | bool | `true` / `false` | Self-destructing first-party cookies flag | And | |
...
These parameters are not used by the report tool but may appear in the URL as part of internal tracking or URL shortener mechanisms:
...
| `app`     | string | `browser_extension`    | App name                                             |

## Scheme versioning

All products must include `scheme_version`. The current version (`3`) defines unified DNS data inclusion from meta information
